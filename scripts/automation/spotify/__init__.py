...
Environment variables (set them or pass explicitly):
  ...
  SPOTIPY_REDIRECT_URI   – redirect URI registered in your app (OAuth only)
...
def get_client(
    *,
    ...
    client_id / client_secret:
        Override the ``SPOTIPY_CLIENT_ID`` / ``SPOTIPY_CLIENT_SECRET`` env vars.
    redirect_uri:
        Override ``SPOTIPY_REDIRECT_URI`` (only used when ``user_auth=True``).
    scopes:
        Space-separated Spotify scopes (only used when ``user_auth=True``).
    """
    cid = client_id or os.environ["SPOTIPY_CLIENT_ID"]
    secret = client_secret or os.environ["SPOTIPY_CLIENT_SECRET"]

    if user_auth:
        uri = redirect_uri or os.environ.get("SPOTIPY_REDIRECT_URI", "http://localhost:8888/callback")
...
def fetch_playlist_by_id(playlist_id: str, *, sp: spotipy.Spotify | None = None) -> dict:
...
def fetch_user_playlists(user_id: str, *, sp: spotipy.Spotify | None = None) -> list[dict]:
